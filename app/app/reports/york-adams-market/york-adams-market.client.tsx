...
export default function YorkAdamsMarketClient() {
  const [selectedMonth, setSelectedMonth] = useState(
    YORK_ADAMS_MONTHLY_DATA[YORK_ADAMS_MONTHLY_DATA.length - 1].month
  );

  const data = useMemo(() => {
    return YORK_ADAMS_MONTHLY_DATA.find((m) => m.month === selectedMonth) ?? YORK_ADAMS_MONTHLY_DATA[YORK_ADAMS_MONTHLY_DATA.length - 1];
  }, [selectedMonth]);
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
      ...
      <div style={{ padding: 16, background: "#eff6ff", borderRadius: 10, border: "1px solid #bfdbfe" }}>
        ...
        <ul style={{ margin: "8px 0 0 16px", padding: 0, fontSize: 12, color: "#374151" }}>
          {data.highlights.map((h, i) => <li key={i} style={{ marginBottom: 3 }}>{h}</li>)}
        </ul>
      </div>

      {/* ═══════ YORK COUNTY ═══════ */}
      <h2 style={{ fontSize: 20, fontWeight: 800, color: COLORS.york, margin: "10px 0 0 0", borderBottom: "3px solid " + COLORS.york, paddingBottom: 6 }}>
        York County
      </h2>

      {/* York Stats */}
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(150px, 1fr))", gap: 10 }}>
        <StatCard label="Median Sold Price" value={fmt(york.medianSoldPrice)} change={york.priceChangeYoY} sub={`Jan 2025: ${fmt(york.medianSoldPricePrevYear)}`} />
        <StatCard label="Homes Sold" value={String(york.homesSold)} change={york.salesChangeYoY} sub={`Jan 2025: ${york.homesSoldPrevYear}`} />
        ...
        <StatCard label="Active Listings" value={String(york.activeListings)} sub={`Jan 2025: ${york.activeListingsPrevYear} (${pct(Math.round((york.activeListings / york.activeListingsPrevYear - 1) * 100))})`} />
        ...
        <table style={{ width: "100%", fontSize: 12, borderCollapse: "collapse" }}>
          <thead>
            <tr style={{ borderBottom: "2px solid #e5e7eb" }}>
              <th style={{ textAlign: "left", padding: "6px 8px", color: "#6b7280", fontWeight: 600 }}>School District</th>
          ...
          <tbody>
            {york.schoolDistricts.map((d, i) => (
              <tr key={d.name} style={{ borderBottom: "1px solid #f3f4f6", background: i % 2 === 0 ? "#fff" : "#f9fafb" }}>
                ...
                <td style={{ padding: "6px 8px", textAlign: "right", fontWeight: 700, color: pctColor(d.priceChangeYoY) }}>{pct(d.priceChangeYoY)}</td>
                ...
                <td style={{ padding: "6px 8px", textAlign: "right", fontWeight: 700, color: pctColor(d.salesChangeYoY) }}>{pct(d.salesChangeYoY)}</td>
              </tr>
            ))}
            <tr style={{ borderTop: "2px solid #1e40af", fontWeight: 800 }}>
              ...
              <td style={{ padding: "6px 8px", textAlign: "right", color: "#6b7280" }}>{fmt(york.medianSoldPricePrevYear)}</td>
              <td style={{ padding: "6px 8px", textAlign: "right", color: pctColor(york.priceChangeYoY) }}>{pct(york.priceChangeYoY)}</td>
              ...
              <td style={{ padding: "6px 8px", textAlign: "right", color: pctColor(york.salesChangeYoY) }}>{pct(york.salesChangeYoY)}</td>
            </tr>
          </tbody>
        </table>
      </div>
      ...
      {/* ═══════ ADAMS COUNTY ═══════ */}
      <h2 style={{ fontSize: 20, fontWeight: 800, color: COLORS.adams, margin: "10px 0 0 0", borderBottom: "3px solid " + COLORS.adams, paddingBottom: 6 }}>
        Adams County
      </h2>

      {/* Adams Stats */}
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(150px, 1fr))", gap: 10 }}>
        <StatCard label="Median Sold Price" value={fmt(adams.medianSoldPrice)} change={adams.priceChangeYoY} sub={`Jan 2025: ${fmt(adams.medianSoldPricePrevYear)}`} />
        <StatCard label="Homes Sold" value={String(adams.homesSold)} change={adams.salesChangeYoY} sub={`Jan 2025: ${adams.homesSoldPrevYear}`} />
        ...
        <StatCard label="Active Listings" value={String(adams.activeListings)} sub={`Jan 2025: ${adams.activeListingsPrevYear} (${pct(Math.round((adams.activeListings / adams.activeListingsPrevYear - 1) * 100))})`} />
        ...
        <table style={{ width: "100%", fontSize: 12, borderCollapse: "collapse" }}>
          ...
          <tbody>
            ...
            <tr style={{ borderTop: "2px solid #dc2626", fontWeight: 800 }}>
              ...
              <td style={{ padding: "6px 8px", textAlign: "right", color: "#6b7280" }}>{fmt(adams.medianSoldPricePrevYear)}</td>
              <td style={{ padding: "6px 8px", textAlign: "right", color: pctColor(adams.priceChangeYoY) }}>{pct(adams.priceChangeYoY)}</td>
              ...
              <td style={{ padding: "6px 8px", textAlign: "right", color: pctColor(adams.salesChangeYoY) }}>{pct(adams.salesChangeYoY)}</td>
            </tr>
          </tbody>
        </table>
      </div>
      ...
      {/* ═══════ COUNTY COMPARISON ═══════ */}
      <h2 style={{ fontSize: 20, fontWeight: 800, color: "#374151", margin: "10px 0 0 0", borderBottom: "3px solid #374151", paddingBottom: 6 }}>
        County Comparison
      </h2>
