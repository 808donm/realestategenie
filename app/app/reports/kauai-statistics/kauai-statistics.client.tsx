...
export default function KauaiStatisticsClient() {
  ...
  const trendData = useMemo(() => {
    return KAUAI_MONTHLY_DATA.map((m) => ({
  ...
  const exportReport = (format: "pdf" | "xlsx") => {
    ...
    if (format === "xlsx") {
      ...
      XLSX.utils.book_append_sheet(wb, ws, "Kauai Statistics");
      XLSX.writeFile(wb, "Kauai_Statistics.xlsx");
    } else {
      const doc = new jsPDF();
      ...
      doc.text("Kauai Statistics", pw / 2, y, { align: "center" });
      y += 8;
      doc.setFontSize(10);
      ...
      <div style={{ display: "flex", gap: 16, marginBottom: 24, flexWrap: "wrap" }}>
        ...
        <div style={{ ...cardStyle, flex: "1 1 380px" }}>
          ...
          <div style={{ textAlign: "center", fontSize: 13, color: "hsl(var(--muted-foreground))" }}>
            Total: {sf.activeListings + cd.activeListings + ld.activeListings} active listings
          </div>
        </div>
      </div>
