...
export default function HawaiiMarketComparisonClient() {
  const [selectedMonth, setSelectedMonth] = useState(STATEWIDE_MONTHLY_DATA[STATEWIDE_MONTHLY_DATA.length - 1].month);

  const statewide = useMemo(() => {
    return (
      STATEWIDE_MONTHLY_DATA.find((m) => m.month === selectedMonth) ??
      STATEWIDE_MONTHLY_DATA[STATEWIDE_MONTHLY_DATA.length - 1]
    );
  }, [selectedMonth]);
  ...
  const hi = useMemo(() => {
    return (
      HAWAII_ISLAND_MONTHLY_DATA.find((m) => m.month === selectedMonth) ??
      HAWAII_ISLAND_MONTHLY_DATA[HAWAII_ISLAND_MONTHLY_DATA.length - 1]
    );
  }, [selectedMonth]);
    ...
    <div>
      ...
      >
        <div>
          ...
          <div style={{ fontSize: 14, opacity: 0.9 }}>Official statewide statistics from Hawai&apos;i Realtors®</div>
        ...
        {hasMultipleMonths && (
          ...
          >
            {STATEWIDE_MONTHLY_DATA.map((m) => (
              <option key={m.month} value={m.month} style={{ color: "#1e293b" }}>
                {m.label}
              </option>
            ))}
          </select>
          ...
          return (
            <div key={sfCounty.county} style={{ ...cardStyle, borderTop: `4px solid ${color}` }}>
              ...
              <div style={{ display: "flex", gap: 16, marginTop: 12, fontSize: 12, color: "hsl(var(--muted-foreground))" }}>
                ...
                <div>
                  <div style={{ fontWeight: 700, color: yoyColor(sfCounty.salesChange) }}>
                    {yoyText(sfCounty.salesChange)}
                  </div>
          ...
          <ResponsiveContainer width="100%" height={260}>
            ...
            >
              ...
              <Bar dataKey="value" name="Days" radius={[6, 6, 0, 0]}>
                {[COLORS.oahu, COLORS.maui, COLORS.hawaii, COLORS.kauai].map((c, i) => (
                  <Cell key={i} fill={c} />
                ))}
              </Bar>
            </BarChart>
        ...
        <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 13 }}>
          ...
          <tbody>
              ...
              >
                ...
                <td
                  style={{
                    ...
                    textAlign: "right",
                    fontWeight: 600,
                    color: yoyColor(c.medianPriceChange),
                  }}
                >
                  {yoyText(c.medianPriceChange)}
                </td>
              </tr>
            ))}
            <tr style={{ borderTop: "2px solid #e5e7eb", fontWeight: 800, background: "#f8fafc" }}>
              ...
              <td
                style={{ padding: "10px 12px", textAlign: "right", color: yoyColor(sw.singleFamily.totalSalesChange) }}
              >
              ...
              <td
                style={{
                  padding: "10px 12px",
                  textAlign: "right",
                  color: yoyColor(sw.singleFamily.totalMedianPriceChange),
                }}
              >
                {yoyText(sw.singleFamily.totalMedianPriceChange)}
              </td>
            </tr>
          </tbody>
        </table>
        ...
        <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 13 }}>
          ...
          <tbody>
            ...
            <tr style={{ borderTop: "2px solid #e5e7eb", fontWeight: 800, background: "#f8fafc" }}>
              ...
              <td
                style={{ padding: "10px 12px", textAlign: "right", color: yoyColor(sw.condo.totalMedianPriceChange) }}
              >
                {yoyText(sw.condo.totalMedianPriceChange)}
              </td>
            </tr>
          </tbody>
        </table>
          ...
          <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 13 }}>
            <thead>
              <tr style={{ borderBottom: "2px solid #e5e7eb" }}>
                ...
                <th style={{ padding: "10px 12px", textAlign: "right", fontWeight: 700, fontSize: 12 }}>
                  SF Sales YoY
                </th>
              ...
              {sw.ytd.singleFamily.counties.map((sfC, i) => {
                ...
                return (
                  <tr
                    ...
                    style={{ borderBottom: "1px solid #f3f4f6", background: i % 2 === 0 ? "#fff" : "#fafafa" }}
                  >
                    <td style={{ padding: "10px 12px", fontWeight: 600, color: colorForCounty(sfC.county) }}>
                      {sfC.county}
                    </td>
                    ...
                    <td
                      style={{
                        ...
                        textAlign: "right",
                        fontWeight: 600,
                        color: yoyColor(sfC.salesChange),
                      }}
                    >
                      {yoyText(sfC.salesChange)}
                    </td>
                    <td style={{ padding: "10px 12px", textAlign: "right", fontWeight: 600 }}>
                      {fmt(sfC.medianPrice2026)}
                    </td>
                    ...
                    <td
                      style={{
                        ...
                        textAlign: "right",
                        fontWeight: 600,
                        color: yoyColor(cdC.salesChange),
                      }}
                    >
                      {yoyText(cdC.salesChange)}
                    </td>
                    <td style={{ padding: "10px 12px", textAlign: "right", fontWeight: 600 }}>
                      {fmt(cdC.medianPrice2026)}
                    </td>
                  </tr>
                );
              })}
              <tr style={{ borderTop: "2px solid #e5e7eb", fontWeight: 800, background: "#f8fafc" }}>
                ...
                <td
                  style={{
                    padding: "10px 12px",
                    textAlign: "right",
                    color: yoyColor(sw.ytd.singleFamily.totalSalesChange),
                  }}
                >
                  {yoyText(sw.ytd.singleFamily.totalSalesChange)}
                </td>
                <td style={{ padding: "10px 12px", textAlign: "right" }}>
                  {fmt(sw.ytd.singleFamily.totalMedianPrice2026)}
                </td>
                <td style={{ padding: "10px 12px", textAlign: "right" }}>{sw.ytd.condo.totalSales2026}</td>
                <td
                  style={{ padding: "10px 12px", textAlign: "right", color: yoyColor(sw.ytd.condo.totalSalesChange) }}
                >
                  {yoyText(sw.ytd.condo.totalSalesChange)}
                </td>
