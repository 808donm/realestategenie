    ...
    (format: "pdf" | "xlsx"): Blob => {
      if (format === "xlsx") {
        ...
        const data: (string | number)[][] = [
          ...
          ["ARV", inputs.arv],
          ["Purchase Price", inputs.purchasePrice],
          ["Rehab", inputs.rehabCost],
          ["Total Investment", analysis.totalInvestment],
          ["Net Profit", analysis.netProfit],
          ["ROI", `${analysis.roi.toFixed(1)}%`],
          ["MAO (70%)", analysis.mao70],
      ...
      [
        ["ARV:", fmt(inputs.arv)],
        ["Purchase:", fmt(inputs.purchasePrice)],
        ["Net Profit:", fmt(analysis.netProfit)],
        ["ROI:", `${analysis.roi.toFixed(1)}%`],
        ["MAO (70%):", fmt(analysis.mao70)],
        ["Verdict:", `${analysis.dealScore}/5 - ${analysis.verdict}`],
      ].forEach(([l, v]) => {
        doc.text(l, 25, y);
        doc.text(v, pw - 25, y, { align: "right" });
    ...
    <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 32 }}>
      ...
      <div>
        ...
        <div style={{ padding: 24, background: "hsl(var(--card))", border: "1px solid hsl(var(--border))", borderRadius: 12 }}>
          ...
          <div style={{ marginBottom: 16 }}>
            ...
            />
            <div style={{ fontSize: 12, color: "hsl(var(--muted-foreground))", marginTop: 4 }}>
              Taxes, insurance, utilities, etc. during hold
            </div>
          </div>
          ...
          <div style={{ marginBottom: 16 }}>
            ...
            />
            <div style={{ fontSize: 12, color: "hsl(var(--muted-foreground))", marginTop: 4 }}>
              Agent commissions, closing costs, transfer tax
            </div>
          </div>
          ...
          <div>
            ...
            <input
              type="number"
              value={inputs.financingCosts}
              onChange={(e) => handleChange("financingCosts", Number(e.target.value))}
              style={inputStyle}
            />
            <div style={{ fontSize: 12, color: "hsl(var(--muted-foreground))", marginTop: 4 }}>Points, interest, loan origination fees</div>
          </div>
        </div>
      </div>
      ...
      <div>
        ...
        >
          ...
          <p style={{ margin: "8px 0 0 0", fontSize: 13, color: "hsl(var(--muted-foreground))" }}>{analysis.verdict}</p>
        </div>
        ...
        >
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12, fontSize: 13 }}>
            <div>
              <div style={{ color: "hsl(var(--muted-foreground))" }}>MAO (70% Rule)</div>
              <div style={{ fontSize: 18, fontWeight: 600 }}>{fmt(analysis.mao70)}</div>
            </div>
            <div>
              <div style={{ color: "hsl(var(--muted-foreground))" }}>Spread</div>
              <div style={{ fontSize: 18, fontWeight: 600, color: analysis.rule70Spread >= 0 ? "#059669" : "#dc2626" }}>
                {analysis.rule70Spread >= 0 ? "+" : ""}
                {fmt(analysis.rule70Spread)}
              </div>
            </div>
          </div>
