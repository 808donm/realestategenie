...
export default async function EditPropertyDetails({ params }: { params: Promise<{ id: string }> }) {
  ...
  if (result.error || !result.data) {
    ...
    const fallback = await supabase
      .from("open_house_events")
      .select("id,address,beds,baths,sqft,price,listing_description,key_features,agent_id,property_photo_url")
      .eq("id", id)
      .single();
    ...
    <div className="max-w-4xl mx-auto p-6">
      <div className="mb-6">
        <div className="flex items-center justify-between">
          ...
          <Link href={`/app/open-houses/${id}`} className="text-sm text-muted-foreground hover:text-foreground">
            ← Back to Open House
          </Link>
        </div>
      </div>
      ...
      <PropertyDetailsForm
        ...
        initialData={{
          ...
          flyer_description: evt.flyer_description,
          flyer_features: evt.flyer_features,
      ...
      <div className="mt-8 p-4 bg-blue-50 border border-blue-200 rounded-lg">
        ...
        <p className="text-sm text-blue-800">
          After saving these details, your professional flyer will automatically include all this information. Download
          it from the open house detail page!
        </p>
      </div>
    </div>
  );
