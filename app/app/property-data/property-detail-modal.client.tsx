            ...
            <>
              ...
              {enrichedFinancial &&
                ...
                (() => {
                  ...
                  const loanBalance =
                    he?.outstandingBalance ?? he?.loanBalance ?? he?.mortgageBalance ?? he?.estimatedBalance;
                  const equityAmount =
                    he?.equity ??
                    he?.equityAmount ??
                    (avmValue != null && loanBalance != null ? avmValue - loanBalance : null);
                  // Compute LTV from values when data source returns 0 or missing
                  const rawLtv = he?.loanToValue ?? he?.ltv;
                  const ltv =
                    rawLtv != null && rawLtv > 0
                      ? rawLtv
                      : avmValue && loanBalance
                        ? (loanBalance / avmValue) * 100
                        : null;
                  ...
                  const lastSaleDate =
                    he?.lastSaleDate ?? he?.saleDate ?? p.sale?.amount?.saleRecDate ?? p.sale?.amount?.saleTransDate;
                  ...
                  return (
                    <div
                      style={{
                        marginBottom: 20,
                        padding: "14px 18px",
                        background: hasData ? (isPositive ? "#ecfdf5" : "#fef2f2") : "#f9fafb",
                        borderRadius: 10,
                        border: `1px solid ${hasData ? (isPositive ? "#a7f3d0" : "#fecaca") : "#e5e7eb"}`,
                      }}
                    >
                      <div
                        style={{
                          fontSize: 11,
                          fontWeight: 600,
                          color: hasData ? (isPositive ? "#059669" : "#dc2626") : "#6b7280",
                          textTransform: "uppercase",
                          letterSpacing: 0.5,
                        }}
                      >
                        ...
                        <>
                          ...
                          >
                            <div>
                              ...
                              <div style={{ fontSize: 16, fontWeight: 700, color: "#111827" }}>
                                {avmValue != null ? fmt(avmValue) : "Not Disclosed"}
                              </div>
                            </div>
                            <div>
                              <div style={{ fontSize: 11, color: "#6b7280", fontWeight: 500 }}>Est. Loan Balance</div>
                              <div style={{ fontSize: 16, fontWeight: 700, color: "#111827" }}>
                                {loanBalance != null ? fmt(loanBalance) : "Not Disclosed"}
                              </div>
                            </div>
                            <div>
                              ...
                              <div
                                style={{
                                  fontSize: 16,
                                  fontWeight: 700,
                                  color: equityAmount != null ? (isPositive ? "#059669" : "#dc2626") : "#9ca3af",
                                }}
                              >
                                {equityAmount != null
                            ...
                            <div>
                              ...
                              <div
                                style={{
                                  fontSize: 16,
                                  fontWeight: 700,
                                  color: ltv != null ? (Number(ltv) > 80 ? "#dc2626" : "#059669") : "#9ca3af",
                                }}
                              >
                          ...
                          >
                            {loanCount != null && loanCount > 0 && (
                              <span>
                                <strong>Active Loans:</strong> {loanCount}
                              </span>
                            )}
                            {estimatedPayment != null && estimatedPayment > 0 && (
                              <span>
                                <strong>Est. Monthly Payment:</strong> {fmt(estimatedPayment)}
                              </span>
                            )}
                            {(() => {
                              ...
                              const saleDate =
                                lastSaleDate ??
                                (mlsSale?.closeDate ? new Date(mlsSale.closeDate).toLocaleDateString() : null);
                              const saleSource =
                                lastSalePrice != null ? "" : mlsSale?.closePrice != null ? " (MLS)" : "";
                              return (
                                <>
                                  <span>
                                    <strong>Last Sale:</strong>{" "}
                                    {salePrice != null ? `${fmt(salePrice)}${saleSource}` : "Not Disclosed"}
                                  </span>
                                  <span>
                                    <strong>Sale Date:</strong> {saleDate || "Not Disclosed"}
                                  </span>
                                  {mlsSale && !lastSalePrice && mlsSale.closePrice && (
                                    <>
                                      {mlsSale.listPrice && (
                                        <span>
                                          <strong>List Price:</strong> {fmt(mlsSale.listPrice)}
                                        </span>
                                      )}
                                      {mlsSale.daysOnMarket != null && (
                                        <span>
                                          <strong>DOM:</strong> {mlsSale.daysOnMarket}
                                        </span>
                                      )}
                                      {mlsSale.listAgentName && (
                                        <span>
                                          <strong>List Agent:</strong> {mlsSale.listAgentName}
                                        </span>
                                      )}
                                      {mlsSale.buyerAgentName && (
                                        <span>
                                          <strong>Buyer Agent:</strong> {mlsSale.buyerAgentName}
                                        </span>
                                      )}
                                    </>
                                  )}
                            ...
                            {p.sale?.amount?.saleRecDate && (
                              <span>
                                <strong>Recording Date:</strong> {p.sale.amount.saleRecDate}
                              </span>
                            )}
                            {p.sale?.amount?.saleDocType && (
                              <span>
                                <strong>Document Type:</strong> {p.sale.amount.saleDocType}
                              </span>
                            )}
                            {p.sale?.amount?.saleCode && (
                              <span>
                                <strong>Sale Code:</strong> {p.sale.amount.saleCode}
                              </span>
                            )}
                            {p.sale?.amount?.pricePerBed != null && (
                              <span>
                                <strong>Price / Bed:</strong> {fmt(p.sale.amount.pricePerBed)}
                              </span>
                            )}
                            {p.sale?.amount?.pricePerSizeUnit != null && (
                              <span>
                                <strong>Price / Sqft:</strong> ${p.sale.amount.pricePerSizeUnit.toFixed(2)}
                              </span>
                            )}
                          </div>
                        </>
