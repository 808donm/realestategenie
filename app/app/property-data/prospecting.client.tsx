...
interface AttomProperty {
  ...
  rentalAvm?: {
    ...
    estimatedMinRentalValue?: number;
    estimatedMaxRentalValue?: number;
    confidenceScore?: number;
    valuationDate?: string;
    rentalAmount?: { value?: number; low?: number; high?: number; scr?: number };
...
function resolveOwner(p: AttomProperty): AttomProperty["owner"] | undefined {
  ...
  const hasTop =
    ...
    top?.owner2?.fullName ||
    top?.owner3?.fullName ||
    top?.corporateIndicator ||
    (top?.mailingAddressOneLine && top.mailingAddressOneLine.trim());
...
function getSaleDateStr(p: AttomProperty): string | null {
  return (
    ...
    p.sale?.amount?.saleRecDate ??
    p.sale?.amount?.saleSearchDate ??
    p.sale?.saleTransDate ??
    p.sale?.saleRecDate ??
    p.sale?.saleSearchDate ??
    null
  );
}
...
function getOwnerName(p: AttomProperty): string | null {
  ...
  return owner?.owner1?.fullName || owner?.owner2?.fullName || owner?.owner3?.fullName || getMortgagorName(p) || null;
}
...
function getPropertyValue(p: AttomProperty): number | null {
  ...
  if (p.assessment?.market?.mktTtlValue != null && p.assessment.market.mktTtlValue > 0)
    return p.assessment.market.mktTtlValue;
  if (p.assessment?.appraised?.apprTtlValue != null && p.assessment.appraised.apprTtlValue > 0)
    return p.assessment.appraised.apprTtlValue;
  if (p.assessment?.assessed?.assdTtlValue != null && p.assessment.assessed.assdTtlValue > 0)
    return p.assessment.assessed.assdTtlValue;
  return null;
}
...
function getDistressSignals(p: AttomProperty): DistressSignals {
  ...
  const assessmentDrop = mktTotal != null && assdTotal != null && assdTotal > 0 ? mktTotal < assdTotal * 0.95 : false;
  ...
  const negativeAppreciation =
    saleAmount != null && saleAmount > 0 && avmValue != null && avmValue > 0 ? avmValue < saleAmount : false;
...
function getOwnershipDate(p: AttomProperty): Date | null {
  ...
  return parseDate(p.mortgage?.date) || parseDate(p.mortgage?.FirstConcurrent?.date) || null;
}
...
function groupByOwner(properties: AttomProperty[]): InvestorGroup[] {
  ...
  for (const [key, props] of groups) {
    ...
    const displayName =
      getOwnerName(props[0]) ||
      (key.startsWith("mail:")
        ? `Unknown Owner (${resolveOwner(props[0])?.mailingAddressOneLine || "Shared Mailing Address"})`
        : "Unknown");
...
export default function Prospecting() {
  ...
  const mapTrestleToAttom = (p: any): AttomProperty => ({
    ...
    address: {
      oneLine: [
        p.UnparsedAddress || p.StreetNumberNumeric
          ? `${p.StreetNumberNumeric || ""} ${p.StreetName || ""} ${p.StreetSuffix || ""}`.trim()
          : null,
        p.City,
        p.StateOrProvince,
        p.PostalCode,
  ...
  const toProspectProperties = (props: AttomProperty[]) =>
    props.map((p) => {
      ...
      return {
        ...
        sqft:
          p.building?.size?.livingSize || p.building?.size?.universalSize || p.building?.size?.bldgSize || undefined,
  ...
  const handleRadiusSearch = async (lat: string, lng: string, address: string) => {
    ...
    try {
      ...
      if (needsExpanded || needsOwner) {
        ...
        suppFetches.push(
          fetch(`/api/integrations/attom/property?${expParams.toString()}`)
            .then((r) => r.json())
            .catch(() => ({ property: [] })),
        );
      }
      if (needsRentalAvm) {
        ...
        suppFetches.push(
          fetch(`/api/integrations/attom/property?${rentalParams.toString()}`)
            .then((r) => r.json())
            .catch(() => ({ property: [] })),
        );
      }
      ...
      if (suppFetches.length > 0) {
        ...
        const suppArrays = suppResults
          .map((r: any) => (r.property || []) as AttomProperty[])
          .filter((a) => a.length > 0);
   ...
   */
  const mergeSupplementalData = (base: AttomProperty[], supplements: AttomProperty[][]): AttomProperty[] => {
    ...
    return base.map((p) => {
      ...
      return {
        ...
        mortgage:
          p.mortgage || supp.mortgage
            ? {
                ...(supp.mortgage || {}),
                ...(p.mortgage || {}),
                // Ensure mortgagor (borrower) fields are preserved from supplement if base lacks them
                borrower1: p.mortgage?.borrower1?.fullName
                  ? p.mortgage.borrower1
                  : (supp.mortgage as any)?.borrower1 || p.mortgage?.borrower1,
                borrower2: p.mortgage?.borrower2?.fullName
                  ? p.mortgage.borrower2
                  : (supp.mortgage as any)?.borrower2 || p.mortgage?.borrower2,
                borrowerVesting: p.mortgage?.borrowerVesting || (supp.mortgage as any)?.borrowerVesting,
                borrowerMailFullStreetAddress:
                  p.mortgage?.borrowerMailFullStreetAddress || (supp.mortgage as any)?.borrowerMailFullStreetAddress,
                ...
                FirstConcurrent:
                  p.mortgage?.FirstConcurrent || (supp.mortgage as any)?.FirstConcurrent
                    ? {
                        ...((supp.mortgage as any)?.FirstConcurrent || {}),
                        ...(p.mortgage?.FirstConcurrent || {}),
                        borrower1: p.mortgage?.FirstConcurrent?.borrower1?.fullName
                          ? p.mortgage.FirstConcurrent.borrower1
                          : (supp.mortgage as any)?.FirstConcurrent?.borrower1,
                        borrower2: p.mortgage?.FirstConcurrent?.borrower2?.fullName
                          ? p.mortgage.FirstConcurrent.borrower2
                          : (supp.mortgage as any)?.FirstConcurrent?.borrower2,
                        borrowerVesting:
                          p.mortgage?.FirstConcurrent?.borrowerVesting ||
                          (supp.mortgage as any)?.FirstConcurrent?.borrowerVesting,
                        borrowerMailFullStreetAddress:
                          p.mortgage?.FirstConcurrent?.borrowerMailFullStreetAddress ||
                          (supp.mortgage as any)?.FirstConcurrent?.borrowerMailFullStreetAddress,
                        borrowerMailCity:
                          p.mortgage?.FirstConcurrent?.borrowerMailCity ||
                          (supp.mortgage as any)?.FirstConcurrent?.borrowerMailCity,
                        borrowerMailState:
                          p.mortgage?.FirstConcurrent?.borrowerMailState ||
                          (supp.mortgage as any)?.FirstConcurrent?.borrowerMailState,
                        borrowerMailZip:
                          p.mortgage?.FirstConcurrent?.borrowerMailZip ||
                          (supp.mortgage as any)?.FirstConcurrent?.borrowerMailZip,
                      }
                    : p.mortgage?.FirstConcurrent,
              }
            : p.mortgage,
        // Fill in missing assessment data from supplement (take richer data)
        assessment: p.assessment?.assessed?.assdTtlValue
          ? p.assessment
          : supp.assessment?.assessed?.assdTtlValue
            ? supp.assessment
            : p.assessment,
        ...
        foreclosure: p.foreclosure?.actionType
          ? p.foreclosure
          : (supp.foreclosure as any)?.actionType
            ? (supp.foreclosure as any)
            : p.foreclosure,
      };
    });
  };
  ...
  const applyModeFilter = (filterMode: ProspectMode, allRawUnfiltered: AttomProperty[], source?: string) => {
    ...
    const withDirectOwner = allRaw.filter((p) => {
      const o = resolveOwner(p);
      return o?.owner1?.fullName || o?.owner2?.fullName || o?.owner3?.fullName;
    }).length;
    const withMortgagor = allRaw.filter((p) => {
      const o = resolveOwner(p);
      return !o?.owner1?.fullName && !o?.owner2?.fullName && !o?.owner3?.fullName && !!getMortgagorName(p);
    }).length;
  ...
  const handleAddressSearch = async () => {
    ...
    try {
      ...
      if (props.length === 0) {
        ...
        setDebugInfo(
          `Found property at: ${props[0].address?.oneLine || farmAddress}. Click to open and view nearby homes for farming.`,
        );
  ...
  const handleSearch = async (pageNum = 1) => {
    ...
    if (mode === "radius") {
      ...
      fetch(
        `/api/integrations/attom/property?endpoint=comparables&postalcode=${zip.trim()}&latitude=0&longitude=0`,
    ...
    try {
      ...
      //   2. "detailowner" — secondary supplement for full owner names + mailing addresses
      {
        ...
        const maxPages = mode === "investor" ? 6 : mode === "foreclosure" ? 5 : mode === "radius" ? 4 : 4;
        ...
        for (let pg = 1; pg <= maxPages; pg++) {
          ...
          if (pg === 1) {
            const withOwner = cacheEnriched.filter((p) => {
              const o = resolveOwner(p);
              return o?.owner1?.fullName || o?.owner2?.fullName;
            }).length;
  ...
  const renderPropertyCard = (prop: AttomProperty, idx: number) => {
    ...
    const heLtv =
      he?.loanToValue ??
      he?.ltv ??
      (he?.avmValue && he?.outstandingBalance
        ? ((he.outstandingBalance ?? he.loanBalance ?? he.mortgageBalance ?? he.estimatedBalance ?? 0) /
            (he.avmValue ?? he.avm?.amount?.value ?? he.estimatedValue ?? 1)) *
          100
        : null);
          ...
          <div style={{ flex: 1, minWidth: 200 }}>
            ...
            {mailingAddr && (
              <div
                style={{
                  fontSize: 12,
                  color: owner ? "#6b7280" : "#374151",
                  fontWeight: owner ? 400 : 600,
                  marginTop: 2,
                }}
              >
            ...
            {(ownerObj?.absenteeOwnerStatus ||
              ...
              <div style={{ marginTop: 4, display: "flex", flexWrap: "wrap", gap: 8, fontSize: 11, color: "#6b7280" }}>
                ...
                {ownerObj?.ownerRelationshipType && <span>{ownerObj.ownerRelationshipType}</span>}
                {saleDateStr && !lastSale && <span>Last transfer: {saleDateStr}</span>}
              </div>
            )}
            ...
            <div style={{ marginTop: 6, display: "flex", flexWrap: "wrap", gap: 12, fontSize: 12, color: "#374151" }}>
              ...
              {rentalVal != null && (
                <span>
                  <strong>Rent Est.:</strong>{" "}
                  <span style={{ color: "#7c3aed" }}>${Number(rentalVal).toLocaleString()}/mo</span>
                </span>
              )}
              {lastSale != null && (
                <span>
                  <strong>Last Sale:</strong> {fmt(lastSale)}
                  {saleDateStr ? ` (${saleDateStr})` : ""}
                </span>
              )}
              {!lastSale && saleDateStr && (
                <span>
                  <strong>Last Transfer:</strong> {saleDateStr} (price not disclosed)
                </span>
              )}
              {mortgageAmt != null && (
                <span>
                  <strong>Mortgage:</strong> {fmt(mortgageAmt)}
              ...
              {heEquity != null ? (
                <span>
                  ...
                  <span style={{ color: heEquity >= 0 ? "#a16207" : "#dc2626", fontWeight: 600 }}>
                    {heEquity >= 0 ? "+" : ""}
                    {fmt(heEquity)}
                  </span>
                  {heLtv != null && (
                ...
                <span>
                  <strong>Est. Equity:</strong>{" "}
                  <span style={{ color: "#a16207", fontWeight: 600 }}>+{fmt(equity)}</span>
                </span>
              ) : null}
              {taxAmt != null && (
                <span>
                  <strong>Tax:</strong> {fmt(taxAmt)}/yr
                </span>
              )}
              {rentalVal != null && avmVal != null && (
                <span>
                  ...
                  <span style={{ color: "#0891b2", fontWeight: 600 }}>
                    {(((rentalVal * 12) / avmVal) * 100).toFixed(1)}%
                  </span>
                </span>
              )}
            </div>
              ...
              <div style={{ marginTop: 6, padding: "6px 10px", background: "#fef2f2", borderRadius: 6, fontSize: 12 }}>
                ...
                <div style={{ display: "flex", flexWrap: "wrap", gap: 12, marginTop: 4 }}>
                  {distress.mortgageAmount != null && (
                    <span>
                      <strong>Mortgage:</strong> {fmt(distress.mortgageAmount)}
                    </span>
                  )}
                  {distress.avmValue != null && (
                    <span>
                      <strong>AVM:</strong> {fmt(distress.avmValue)}
                    </span>
                  )}
                  {distress.ltvPct != null && (
                    <span
                      style={{
                        color: distress.ltvPct >= 100 ? "#dc2626" : distress.ltvPct >= 80 ? "#d97706" : "#374151",
                        fontWeight: 600,
                      }}
                    >
                ...
                {distress.negativeAppreciation && lastSale != null && avmVal != null && (
                  <div style={{ marginTop: 4, color: "#be123c", fontWeight: 600, fontSize: 11 }}>
                    Value decline: purchased for {fmt(lastSale)}, now worth {fmt(avmVal)} (
                    {(((avmVal - lastSale) / lastSale) * 100).toFixed(1)}%)
                  </div>
                )}
                {distress.minimalAppreciation &&
                  !distress.negativeAppreciation &&
                  yearsOwned != null &&
                  lastSale != null &&
                  avmVal != null && (
                    <div style={{ marginTop: 4, color: "#7c3aed", fontWeight: 600, fontSize: 11 }}>
                      Stagnant: owned {yearsOwned} years, only {(((avmVal - lastSale) / lastSale) * 100).toFixed(0)}%
                      appreciation (purchased {fmt(lastSale)})
                    </div>
                  )}
              </div>
            )}
              ...
              >
                ...
                <div style={{ display: "flex", flexWrap: "wrap", gap: 12 }}>
                  {prop.foreclosure.filingDate && (
                    <span>
                      <strong>Filed:</strong> {prop.foreclosure.filingDate}
                    </span>
                  )}
                  {prop.foreclosure.recordingDate && (
                    <span>
                      <strong>Recorded:</strong> {prop.foreclosure.recordingDate}
                    </span>
                  )}
                  {prop.foreclosure.auctionDate && (
                    <span>
                      <strong>Auction:</strong>{" "}
                      <span style={{ color: "#dc2626", fontWeight: 600 }}>{prop.foreclosure.auctionDate}</span>
                    </span>
                  )}
                  {prop.foreclosure.defaultAmount != null && (
                    <span>
                      <strong>Default:</strong> {fmt(prop.foreclosure.defaultAmount)}
                    </span>
                  )}
                  {prop.foreclosure.originalLoanAmount != null && (
                    <span>
                      <strong>Original Loan:</strong> {fmt(prop.foreclosure.originalLoanAmount)}
                    </span>
                  )}
                  {prop.foreclosure.startingBid != null && (
                    <span>
                      <strong>Starting Bid:</strong>{" "}
                      <span style={{ color: "#059669", fontWeight: 600 }}>{fmt(prop.foreclosure.startingBid)}</span>
                    </span>
                  )}
                  {prop.foreclosure.trusteeFullName && (
                    <span>
                      <strong>Trustee:</strong> {prop.foreclosure.trusteeFullName}
                    </span>
                  )}
                  {prop.foreclosure.auctionLocation && (
                    <span>
                      <strong>Location:</strong> {prop.foreclosure.auctionLocation}
                    </span>
                  )}
                </div>
              </div>
            ...
            {mode === "radius" && (
              <div style={{ marginTop: 6, padding: "6px 10px", background: "#f5f3ff", borderRadius: 6, fontSize: 12 }}>
                <div style={{ display: "flex", flexWrap: "wrap", gap: 12, alignItems: "center" }}>
                  ...
                  {saleDateStr && (
                    <span>
                      <strong>Date:</strong> {saleDateStr}
                    </span>
                  )}
                  ...
                  {prop.location?.latitude && prop.location?.longitude && (
                    <button
                      ...
                      style={{
                        ...
                        fontSize: 11,
                        fontWeight: 600,
                        cursor: radiusLoading ? "not-allowed" : "pointer",
          ...
          <div style={{ display: "flex", gap: 16, flexWrap: "wrap", textAlign: "right" }}>
              ...
              <div>
                ...
                <div
                  style={{
                    fontWeight: 700,
                    fontSize: 15,
                    color: distress.ltvPct >= 100 ? "#dc2626" : distress.ltvPct >= 80 ? "#d97706" : "#059669",
                  }}
                >
            ...
            {rentalVal != null && (
              <div>
                ...
                <div style={{ fontWeight: 700, fontSize: 15, color: "#7c3aed" }}>
                  ${Number(rentalVal).toLocaleString()}/mo
                </div>
                {rentalVal != null && avmVal != null && (
          ...
          >
            ...
            <div style={{ display: "flex", gap: 16, flexWrap: "wrap", textAlign: "right" }}>
              ...
              <div>
                ...
                <div
                  style={{ fontWeight: 700, fontSize: 15, color: group.totalTaxBurden > 20000 ? "#dc2626" : "#374151" }}
                >
              ...
              {group.totalRentalIncome > 0 && (
                <div>
                  ...
                  <div style={{ fontWeight: 700, fontSize: 15, color: "#7c3aed" }}>
                    ${group.totalRentalIncome.toLocaleString()}
                  </div>
  ...
  return (
    <div>
          ...
          m.id === ("dom" as ProspectMode) ? (
            ...
            <button
              ...
              onClick={() => {
                ...
                if (
                  newMode !== "radius" &&
                  sharedRawData.length > 0 &&
                  sharedDataKey === `${zip.trim()}:${propertyType}`
                ) {
      ...
      <div style={{ background: "#fff", border: "1px solid #e5e7eb", borderRadius: 12, padding: 20, marginBottom: 20 }}>
        ...
        <div style={{ display: "flex", gap: 12, flexWrap: "wrap", alignItems: "flex-end" }}>
            ...
            <div>
              ...
              <input
                ...
                onChange={(e) => {
                  ...
                  if (sharedDataKey || propertyCacheKey) {
                    setSharedRawData([]);
                    setSharedDataKey("");
          ...
          <button
            ...
            style={{
              padding: "8px 24px",
              background: modes.find((m) => m.id === mode)?.color || "#3b82f6",
              color: "#fff",
              borderRadius: 8,
              border: "none",
              fontWeight: 600,
          ...
          <div style={{ position: "relative", display: "flex", alignItems: "center", gap: 4 }}>
            <button
              onClick={() => {
                if (aiPanelRef.current) {
                  ...
                  setTimeout(() => {
                    document
                      .getElementById("prospect-ai-panel")
              ...
              style={{
                padding: "8px 20px",
                background:
                  hasSearched && (results.length > 0 || investorGroups.length > 0)
            ...
            {showAITooltip && (
              ...
              >
                ...
                <p style={{ margin: "0 0 8px" }}>The AI engine analyzes your search results to:</p>
                <ul style={{ margin: "0 0 8px", paddingLeft: 18 }}>
                  <li>
                    <strong>Score &amp; rank</strong> each prospect 0–100 based on seller motivation signals
                  </li>
                  <li>
                    <strong>Categorize</strong> into Hot, Warm, and Long-term tiers
                  </li>
                  <li>
                    <strong>Explain</strong> why each property ranks where it does using actual data
                  </li>
                  <li>
                    <strong>Draft outreach</strong> — personalized letters, SMS, and talking points
                  </li>
                  <li>
            ...
            >
              ...
              >
                ...
                {priceChange != null && (
                  <span style={{ fontWeight: 600, fontSize: 12, color: priceChange >= 0 ? "#059669" : "#dc2626" }}>
                    {priceChange >= 0 ? "+" : ""}
                    {priceChange.toFixed(1)}% median price (last year)
                  </span>
                )}
              </div>
              <div style={{ display: "flex", flexWrap: "wrap", gap: 16, marginTop: 6 }}>
                {st?.medSalePrice != null && (
                  <span>
                    <strong>Median:</strong> ${Number(st.medSalePrice).toLocaleString()}
                  </span>
                )}
                {st?.avgSalePrice != null && (
                  <span>
                    <strong>Avg:</strong> ${Number(st.avgSalePrice).toLocaleString()}
                  </span>
                )}
                {st?.homeSaleCount != null && (
                  <span>
                    <strong>Sales:</strong> {st.homeSaleCount}
                  </span>
                )}
                {st?.avgDaysOnMarket != null && (
                  <span>
                    <strong>Avg DOM:</strong> {st.avgDaysOnMarket} days
                  </span>
                )}
                {st?.medPricePerSqFt != null && (
                  <span>
                    <strong>Med $/sqft:</strong> ${Number(st.medPricePerSqFt).toLocaleString()}
                  </span>
                )}
              </div>
            </div>
            ...
            <>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                {mlsResults.map((prop, idx) => {
                  ...
                  const spLpRatio =
                    closePrice && listPrice && listPrice > 0 ? ((closePrice / listPrice) * 100).toFixed(1) : null;
                        ...
                        <div style={{ flex: 1 }}>
                          ...
                          >
                            <div style={{ display: "flex", flexWrap: "wrap", gap: 12 }}>
                              {closePrice != null && (
                                <span>
                                  <strong>Sold:</strong> {fmt(closePrice)}
                                </span>
                              )}
                              {listPrice != null && (
                                <span>
                                  <strong>List:</strong> {fmt(listPrice)}
                                </span>
                              )}
                              {spLpRatio && (
                                <span>
                                  ...
                                  <span
                                    style={{
                                      color: parseFloat(spLpRatio) >= 100 ? "#059669" : "#dc2626",
                                      fontWeight: 600,
                                    }}
                                  >
                                    {spLpRatio}%
                              ...
                              {closeDate && (
                                <span>
                                  <strong>Closed:</strong> {new Date(closeDate).toLocaleDateString()}
                                </span>
                              )}
                              {dom != null && (
                                <span>
      ...
      {mode === "radius" && hasSearched && mlsError && (
        ...
        >
          <strong>MLS Data:</strong>{" "}
          {mlsError === "Trestle MLS is not connected. Go to Integrations to set it up."
            ? "Connect Trestle MLS in Integrations to see closed listing data with photos, agent info, and SP/LP ratios."
            : mlsError}
        </div>
      )}
                  ...
                  >
                    ...
                    <div style={{ display: "flex", flexWrap: "wrap", gap: 12, color: "#6b7280" }}>
                      ...
                      {getMailingAddress(prop) && (
                        <span>
                          {resolveOwner(prop)?.mailingAddressOneLine?.trim() ? "Mailing" : "Mortgagor Address"}:{" "}
                          {getMailingAddress(prop)}
                        </span>
                      )}
                      {getPropertyValue(prop) != null && (
                        <span>
                          <strong>Value:</strong> {fmt(getPropertyValue(prop)!)}
                        </span>
                      )}
                      {getMortgageAmount(prop) != null && (
                        <span>
                          <strong>Mortgage:</strong> {fmt(getMortgageAmount(prop)!)}
                        </span>
                      )}
                      {getMortgageLender(prop) && (
                        <span>
                          <strong>Lender:</strong> {getMortgageLender(prop)}
                        </span>
                      )}
                    </div>
                  </div>
      ...
      {!hasSearched && !isLoading && (
        <div style={{ padding: 40, textAlign: "center", color: "#9ca3af", background: "#f9fafb", borderRadius: 12 }}>
          ...
          <div style={{ fontSize: 13, maxWidth: 500, margin: "0 auto" }}>
            {mode === "absentee" &&
              "Search for non-owner-occupied properties by zip code. These owners are managing properties remotely and may be motivated to sell."}
            {mode === "equity" &&
              'Search for long-tenure homeowners based on their purchase date. Owners who bought 10+ years ago have built significant equity through appreciation — ideal for "unlock your equity" listing campaigns.'}
            {mode === "foreclosure" &&
              "Find properties with underwater mortgages, high loan-to-value ratios, or declining assessments. These owners may be under financial pressure and open to selling."}
            {mode === "radius" &&
              "Search MLS closed listings and property records by zip code or address. Results combine Trestle MLS data (sale price, SP/LP ratio, agent info, photos) with Realie records (owner names, mailing addresses, valuations). Click any sold property to find nearby homeowners for postcard campaigns."}
            {mode === "investor" &&
              "Find corporate entities, absentee owners, and multi-property investors in a zip code. Shows owner contact info, mailing addresses, mortgage details, and estimated values."}
          </div>
        </div>
      )}
