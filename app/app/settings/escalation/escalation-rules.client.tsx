              ...
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
                <div style={{ flex: 1 }}>
                  <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 4 }}>
                    ...
                    <span style={{
                      ...
                      background: actionStyle.bg, color: actionStyle.color, textTransform: "uppercase",
                    }}>
                  ...
                  <div style={{ display: "flex", gap: 8, marginTop: 8, flexWrap: "wrap" }}>
                    <span style={{ fontSize: 11, padding: "2px 8px", background: "#f3f4f6", borderRadius: 4, color: "#374151" }}>
                      Trigger: {TRIGGER_LABELS[rule.trigger] || rule.trigger}
                    </span>
                    {rule.threshold != null && (
                      <span style={{ fontSize: 11, padding: "2px 8px", background: "#fef3c7", borderRadius: 4, color: "#a16207" }}>
                        Threshold: {rule.threshold}
                      </span>
                    )}
                    {rule.keywords && rule.keywords.length > 0 && (
                      <span style={{ fontSize: 11, padding: "2px 8px", background: "#e0e7ff", borderRadius: 4, color: "#3730a3" }}>
                        Keywords: {rule.keywords.slice(0, 3).join(", ")}{rule.keywords.length > 3 ? ` +${rule.keywords.length - 3}` : ""}
                      </span>
                    )}
                  </div>
                </div>
                <label style={{ position: "relative", display: "inline-block", width: 44, height: 24, flexShrink: 0, marginLeft: 12 }}>
                  ...
                  <span style={{
                    ...
                    background: rule.enabled ? "#3b82f6" : "#d1d5db", borderRadius: 24, transition: "0.3s",
                  }}>
                    <span style={{
                      position: "absolute", height: 18, width: 18, left: rule.enabled ? 22 : 3, bottom: 3,
