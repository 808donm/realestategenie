...
export default async function ProfilePage() {
  ...
  const { data: agent, error } = await supabase
    .from("agents")
    .select(
      "id,email,display_name,license_number,agency_name,phone_e164,locations_served,photo_url,landing_page,headshot_url,company_logo_url,timezone",
    )
  ...
  async function updateLandingPage(formData: FormData) {
    ...
    await supabase.from("agents").update({ landing_page }).eq("id", data.user.id);
  }

  return (
    <div style={{ maxWidth: 720, margin: "40px auto", padding: 16 }}>
      <h1 style={{ fontSize: 28, fontWeight: 700 }}>Agent Profile</h1>
      <p style={{ opacity: 0.75 }}>This info appears on your open house pages.</p>
