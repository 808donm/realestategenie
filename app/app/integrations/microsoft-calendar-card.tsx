...
export default function MicrosoftCalendarCard({ integration }: { integration: Integration }) {
  const [disconnecting, setDisconnecting] = useState(false);
  const isConnected = integration?.status === "connected";
  const hasError = integration?.status === "error";

  const handleConnect = () => {
    window.location.href = "/api/integrations/microsoft-calendar/connect";
  };

  const handleDisconnect = async () => {
    if (!confirm("Disconnect Microsoft Calendar? Two-way sync will stop.")) return;

    setDisconnecting(true);
    try {
      const response = await fetch("/api/integrations/microsoft-calendar/disconnect", {
        method: "POST",
      });
    ...
    <Card>
      <CardHeader>
        <div className="flex items-start justify-between">
          <div className="flex items-center gap-3">
            <div className="w-12 h-12 bg-gradient-to-br from-green-400 to-teal-600 rounded-lg flex items-center justify-center">
      ...
      <CardContent className="space-y-4">
        <p className="text-sm text-muted-foreground">
          Sync events between your Microsoft/Outlook Calendar and the merged calendar. Outlook takes precedence on
          conflicts.
        </p>
        ...
        <div className="text-xs text-muted-foreground">
          <strong>Features:</strong> Two-way event sync, Graph API subscriptions, conflict resolution (Outlook wins)
        </div>
      </CardContent>
    </Card>
  );
