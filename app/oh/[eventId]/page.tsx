...
export default async function OpenHouseIntakePage({
  ...
  if (!token) {
    return (
      <div style={{ maxWidth: 720, margin: "40px auto", padding: 16 }}>
        ...
        <p style={{ marginTop: 12 }}>This open house registration page requires a valid access token.</p>
        ...
        <div style={{ marginTop: 24, padding: 16, background: "#f0f0f0", borderRadius: 8 }}>
          ...
          <p style={{ marginTop: 8, fontSize: 14 }}>
            This page is protected to ensure that only attendees at the physical open house can register. If you're at
            the open house, please scan the QR code provided by the agent.
          </p>
        </div>
      </div>
    );
  ...
  if (!tokenValidation.valid) {
    return (
      <div style={{ maxWidth: 720, margin: "40px auto", padding: 16 }}>
        ...
        <p style={{ marginTop: 12 }}>{tokenValidation.error || "The access token is invalid or has expired."}</p>
        <p style={{ marginTop: 8, opacity: 0.75 }}>Please scan the QR code again at the open house entrance.</p>
  ...
  if (tokenValidation.payload?.eventId !== eventId) {
    return (
      <div style={{ maxWidth: 720, margin: "40px auto", padding: 16 }}>
        ...
        <p style={{ marginTop: 12 }}>This access token is for a different open house event.</p>
        <p style={{ marginTop: 8, opacity: 0.75 }}>Please scan the QR code for this specific open house.</p>
      </div>
    );
  }
  ...
  if (error || !event) {
    return (
      <div style={{ maxWidth: 720, margin: "40px auto", padding: 16 }}>
        ...
        <p style={{ color: "crimson" }}>This open house is not available. (It may be unpublished or expired.)</p>
        ...
        <div style={{ marginTop: 20, padding: 16, background: "#f0f0f0", borderRadius: 8, fontSize: 12 }}>
          ...
          <div>
            <strong>View Error:</strong> {error?.message || "Event not found in view"}
          </div>
          {debugEvent && (
            <>
              <div style={{ marginTop: 8 }}>
                <strong>Event Status:</strong> {debugEvent.status}
              </div>
              <div>
                <strong>Event Address:</strong> {debugEvent.address}
              </div>
              <div>
                <strong>Agent ID:</strong> {debugEvent.agent_id}
              </div>
  ...
  const geocodedLocation = !event.latitude || !event.longitude ? await geocodeAddress(event.address) : null;
  const resolvedLatitude = event.latitude ?? geocodedLocation?.latitude ?? null;
  const resolvedLongitude = event.longitude ?? geocodedLocation?.longitude ?? null;
  ...
  return (
    <div style={{ maxWidth: 720, margin: "40px auto", padding: 16 }}>
      ...
      <div style={{ display: "flex", gap: 14, alignItems: "center" }}>
        ...
        <div>
          <div style={{ fontSize: 18, fontWeight: 800 }}>{event.display_name || "Your Agent"}</div>
      ...
      <h1 style={{ fontSize: 26, fontWeight: 800, marginTop: 18 }}>{event.address}</h1>
