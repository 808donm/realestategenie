...
function Row({ label, value }: { label: string; value?: string | number | null }) {
  ...
  return (
    <div
      style={{ display: "flex", justifyContent: "space-between", padding: "4px 0", borderBottom: "1px solid #f3f4f6" }}
    >
    ...
    <div style={{ minHeight: "100vh", background: "#f9fafb" }}>
      ...
      <div style={{ background: "linear-gradient(135deg, #1e40af 0%, #3b82f6 100%)", padding: "32px 0 24px" }}>
        <div style={{ maxWidth: 760, margin: "0 auto", padding: "0 20px" }}>
          ...
          <div style={{ fontSize: 12, color: "rgba(255,255,255,0.6)", marginTop: 8 }}>
            Prepared by {b.displayName}
            {b.licenseNumber ? ` (Lic# ${b.licenseNumber})` : ""}
            {" | "}
      ...
      <div style={{ maxWidth: 760, margin: "0 auto", padding: "24px 20px 60px" }}>
        ...
        <div style={{ display: "flex", gap: 12, flexWrap: "wrap", marginBottom: 28 }}>
          {d.avmValue != null && (
            <Card label="AVM Value" value={fmt(d.avmValue)!} sub={d.avmDate ? `As of ${d.avmDate}` : undefined} />
          )}
          {d.lastSalePrice != null && (
            <Card label="Last Sale" value={fmt(d.lastSalePrice)!} sub={d.lastSaleDate || undefined} color="#eff6ff" />
          )}
        ...
        {d.federalData && (
          <Section title="Neighborhood & Economic Context">
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "0 32px" }}>
              ...
              <Row
                label="Median Age"
                value={d.federalData.medianAge != null ? String(d.federalData.medianAge) : null}
              />
              <Row
                label="Population Density"
                value={
                  d.federalData.populationDensity != null
                    ? `${d.federalData.populationDensity.toLocaleString()} /sq mi`
                    : null
                }
              />
