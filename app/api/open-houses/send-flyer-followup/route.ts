...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!lead.ghl_contact_id) {
      return NextResponse.json({ error: "Lead not synced to GHL yet. Please sync first." }, { status: 400 });
    }

    const event = Array.isArray(lead.open_house_events) ? lead.open_house_events[0] : lead.open_house_events;
    ...
    if (integrationError || !integration) {
      return NextResponse.json({ error: "GHL integration not found or not connected" }, { status: 400 });
    }
    ...
    try {
      ...
      return NextResponse.json({ error: "Failed to send SMS", details: smsError.message }, { status: 500 });
    }
    ...
    return NextResponse.json({ error: "Failed to send follow-up", details: error.message }, { status: 500 });
  }
}
