...
import { RentcastClient, mapAttomParamsToRentcast, mapRentcastToAttomShape } from "@/lib/integrations/rentcast-client";
import { mapAttomParamsToRealie, mapRealieToAttomShape } from "@/lib/integrations/realie-client";
 ...
 */
async function fetchFromRealie(endpoint: string, params: Record<string, any>): Promise<any | null> {
 ...
 */
function mergePropertyData(rentcastProps: any[], realieProps: any[]): any[] {
  ...
  return rentcastProps.map((rcProp) => {
    ...
    // AVM — always prefer Realie's model value when available. Realie provides
    // a dedicated AVM model (modelValue/min/max) with confidence ranges sourced
    // from county records. RentCast's "AVM" is just lastSalePrice (stale).
    // When MLS listing price is available it will supersede both in the UI.
    const realieAvm = realieProp.avm?.amount?.value;
    const realieHasRange = realieProp.avm?.amount?.high && realieProp.avm?.amount?.low;
    if (realieAvm && realieHasRange) {
      // Realie has a real AVM model with confidence range — always use it
      merged.avm = realieProp.avm;
      console.log(
        `[Merge] AVM: using Realie model $${realieAvm.toLocaleString()} (${realieProp.avm.amount.low?.toLocaleString()} – ${realieProp.avm.amount.high?.toLocaleString()})`,
      );
    } else if (realieAvm) {
      // Realie has a value but no range — still prefer it over lastSalePrice
      merged.avm = realieProp.avm;
    }
    // else: no Realie AVM, keep RentCast's lastSalePrice proxy
    ...
    // Sale history — prefer Realie's transfers[] (has buyer/seller names, deed types).
    // Fall back to RentCast's history dict (has prices but no names).
    // Realie is the authoritative source since Hawaii is a non-disclosure state
    // and sale prices come directly from county deed records.
    if (realieProp.saleHistory?.length) {
      merged.saleHistory = realieProp.saleHistory;
    } else if (!merged.saleHistory?.length && rcProp.saleHistory?.length) {
      merged.saleHistory = rcProp.saleHistory;
    }
 ...
 */
async function fetchFromRentcast(endpoint: string, params: Record<string, any>): Promise<any | null> {
 ...
 */
async function fetchFromFreeData(endpoint: string, params: Record<string, any>): Promise<any> {
  const lat = params.latitude;
  const lng = params.longitude;
  const postalCode = params.postalcode || params.postalCode;
  const state =
    params.address2?.match(/\b([A-Z]{2})\b/)?.[1] || params.address?.match(/,\s*([A-Z]{2})\s*\d{0,5}\s*$/)?.[1];
  ...
  switch (endpoint) {
    ...
    case "community": {
      const crime = fips ? await getCrimeIndicesByFips(fips) : state ? await getCrimeIndicesByState(state) : null;
...
export async function GET(request: NextRequest) {
  try {
    ...
    const isCachedDataUsable = (cached: any): boolean => {
      ...
      const withValue = sample.filter(
        (p: any) => p.avm?.amount?.value || p.assessment?.market?.mktTtlValue || p.assessment?.assessed?.assdTtlValue,
      ).length;
    ...
    if (memoryCached && isCachedDataUsable(memoryCached.data)) {
      return NextResponse.json(
        { ...memoryCached.data, dataSource: memoryCached.source, cacheHit: "memory" },
        {
    ...
    if (FREE_DATA_ENDPOINTS.has(endpoint)) {
      ...
      if (!stateHint || !isNonDisclosureState(stateHint)) {
        return NextResponse.json(
          {
            success: false,
            error: `Comp Genie is only available for non-disclosure states. ${stateHint || "Unknown"} is not a non-disclosure state.`,
          },
          { status: 400 },
        );
      }
      ...
      const toCompProp = (p: any): CompProperty => ({
        ...
        pricePerSqft:
          p.avm?.amount?.value && (p.building?.size?.universalSize || p.building?.size?.livingSize)
        ...
        return NextResponse.json(
          {
            ...
            dataSource: "none",
            property: [],
            message: "No property data provider returned results",
          },
        ...
        return NextResponse.json(
          {
            success: false,
            endpoint,
            error: `Endpoint "${endpoint}" has been deprecated. Use RentCast property data or free data sources instead.`,
            property: [],
          },
          { status: 200 },
        );
      ...
      if (rcResult?.property?.length > 0) {
        ...
        return NextResponse.json({ error: "No property data provider configured" }, { status: 503 });
      }
    }
