...
export async function GET(request: NextRequest) {
  try {
    ...
    if (FREE_DATA_ENDPOINTS.has(endpoint)) {
      ...
      // ── Comp Genie: AI-powered comparable analysis (non-disclosure states only)
      const stateHint =
        params.countrySubd || params.state ||
        params.address2?.match(/\b([A-Z]{2})\b/)?.[1];
      if (stateHint && !isNonDisclosureState(stateHint)) {
        return NextResponse.json(
          { success: false, endpoint, error: `${stateHint} is a disclosure state — use the standard comparables endpoint instead.`, property: [] },
          { status: 200 },
        );
      }
      ...
      if (!lat || !lng) {
        return NextResponse.json(
          { success: false, endpoint, error: "Comp Genie requires latitude and longitude for the subject property.", property: [] },
          { status: 200 },
        );
      }

      // Fetch subject + neighbors via radius search
      const realieResult = await fetchFromRealie("expanded", {
        ...params,
        latitude: String(lat),
      ...
      if (!realieResult?.property?.length) {
        return NextResponse.json(
          { success: false, endpoint, error: "No properties found near the subject for comparable analysis.", property: [] },
          { status: 200 },
        );
      }

      // Import and run Comp Genie
      const { analyzeComparables } = await import("@/lib/ai/comp-genie");
      ...
      // Subject is the closest property to the requested lat/lng (or first result)
      const props = realieResult.property as any[];
      const subjectProp = props[0];
      const subject = toCompProp(subjectProp);
      const neighbors = props.slice(1).map(toCompProp);

      const compResult = await analyzeComparables(subject, neighbors, stateHint || "");
      // Return directly — don't cache AI analysis
      return NextResponse.json(
        { success: true, endpoint, dataSource: "ai", property: [subjectProp], compGenie: compResult },
        { status: 200 },
      );
