...
export async function GET(request: NextRequest) {
  ...
  if (fetchError) {
    ...
    return NextResponse.redirect(
      `${redirectBase}?error=microsoft_invalid_state&message=${encodeURIComponent("DB query failed: " + fetchError.message)}`,
    );
  }

  // Find the integration whose config.oauth_state matches
  const integration = integrations?.find((i: any) => i.config?.oauth_state === state);

  if (!integration) {
    console.error("[Microsoft Calendar] No integration found for state:", state, "Found rows:", integrations?.length);
    return NextResponse.redirect(
      `${redirectBase}?error=microsoft_invalid_state&message=${encodeURIComponent("No matching state found. Found " + (integrations?.length || 0) + " disconnected rows.")}`,
    );
  }
  ...
  try {
    ...
    if (!response.ok) {
      ...
      return NextResponse.redirect(
        `${redirectBase}?error=microsoft_token_failed&message=${encodeURIComponent("Token exchange failed (" + response.status + "): " + err.substring(0, 200))}`,
      );
    }
    ...
    if (updateError) {
      ...
      return NextResponse.redirect(
        `${redirectBase}?error=microsoft_oauth_failed&message=${encodeURIComponent("Failed to save tokens: " + updateError.message)}`,
      );
    }
    ...
    return NextResponse.redirect(
      `${redirectBase}?error=microsoft_oauth_failed&message=${encodeURIComponent(err.message || String(err))}`,
    );
  }
}
