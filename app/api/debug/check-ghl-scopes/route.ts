...
export async function GET(req: Request) {
  try {
    ...
    if (!agentId) {
      ...
      const { data: agent } = await admin.from("agents").select("id").limit(1).single();
    ...
    return NextResponse.json({
      ...
      diagnosis: {
        ...
        canCreateAssociations: hasAssociationsRead
          ? "Likely yes (has read access)"
          : "❌ No - missing associations.readonly/write scopes",
        hasMessagesAccess: hasMessagesRead,
        canSendEmails: hasMessagesRead
          ? "✅ Yes (has messages access)"
          : "❌ No - missing conversations/message scopes",
