...
export async function GET(req: Request) {
  try {
    ...
    if (!agentId) {
      ...
      const { data: agents } = await admin.from("agents").select("id").limit(1).single();
    ...
    const openHousesObj = customObjects.find(
      (obj: any) => obj.name?.toLowerCase().includes("openhouse") || obj.objectKey?.toLowerCase().includes("openhouse"),
    );

    const registrationsObj = customObjects.find(
      (obj: any) =>
        obj.name?.toLowerCase().includes("registration") || obj.objectKey?.toLowerCase().includes("registration"),
    );
    ...
    const openHousesSchema = openHousesSchemaResponse.ok ? await openHousesSchemaResponse.json() : null;

    const registrationsSchema = registrationsSchemaResponse.ok ? await registrationsSchemaResponse.json() : null;
    ...
    return NextResponse.json({
      ...
      openHouses: {
        ...
        id: openHousesObj.id,
        fields:
          openHousesSchema?.customObject?.fields?.map((field: any) => ({
            name: field.name,
            key: field.key,
            type: field.type,
      ...
      registrations: {
        ...
        fields:
          registrationsSchema?.customObject?.fields?.map((field: any) => ({
            name: field.name,
            key: field.key,
            type: field.type,
            required: field.required,
