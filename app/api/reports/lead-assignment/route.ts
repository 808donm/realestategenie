...
export async function GET() {
  ...
  const perAgent = new Map<
    string,
    { received: number; contacted: number; converted: number; responseMins: number[] }
  >();
  ...
  (leads || []).forEach((l) => {
    ...
    if (l.pipeline_stage !== "new_lead") {
      ...
      if (l.updated_at && l.created_at) {
        entry.responseMins.push(
          Math.max(0, (new Date(l.updated_at).getTime() - new Date(l.created_at).getTime()) / 60000),
        );
      }
    }
  ...
  const data = agentIds
    .map((id) => {
      ...
      const avgResp =
        e.responseMins.length > 0 ? Math.round(e.responseMins.reduce((a, b) => a + b, 0) / e.responseMins.length) : 0;
      return {
