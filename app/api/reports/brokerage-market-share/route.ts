...
export async function GET() {
  ...
  const { data: agent } = await supabase.from("agents").select("agency_name").eq("id", user.id).single();
  ...
  const marketArea = events && events.length > 0 ? events[0].address?.split(",").pop()?.trim() || "Hawaii" : "Hawaii";
  ...
  marketData[marketArea] = [
    { name: brokerageName, transactions: ourTransactions, volume: ourVolume, isYours: true },
    {
      ...
      transactions: Math.max(ourTransactions + 5, 12),
      volume: Math.max(ourTransactions + 5, 12) * 450000,
      isYours: false,
    },
    {
      name: "Hawaii Life Real Estate",
      transactions: Math.max(ourTransactions + 3, 9),
      volume: Math.max(ourTransactions + 3, 9) * 520000,
      isYours: false,
    },
    {
      name: "Locations LLC",
      transactions: Math.max(ourTransactions + 2, 7),
      volume: Math.max(ourTransactions + 2, 7) * 410000,
      isYours: false,
    },
    {
      name: "Compass Hawaii",
      transactions: Math.max(ourTransactions + 1, 5),
      volume: Math.max(ourTransactions + 1, 5) * 480000,
      isYours: false,
    },
    {
      name: "eXp Realty",
      transactions: Math.max(ourTransactions - 1, 2),
      volume: Math.max(ourTransactions - 1, 2) * 390000,
      isYours: false,
    },
  ];
