  ...
  try {
    ...
    const report = {
      ...
      property: {
        ...
        lotSize: property.lot?.lotSize2 ? `${property.lot.lotSize2.toFixed(2)} acres` : property.lot?.lotSize1 ? `${property.lot.lotSize1.toLocaleString()} sqft` : null,
      ...
      // Estimated Equity
      estimatedEquity: property.avm?.amount?.value && (property.sale?.amount?.saleAmt || property.sale?.amount?.salePrice)
