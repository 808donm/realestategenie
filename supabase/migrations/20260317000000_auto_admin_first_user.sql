-- Migration: Auto-admin first user
-- When the first agent is created on a new site, they should automatically be admin.
...
BEGIN
  ...
  -- Check if this is the first agent (auto-admin)
...
COMMENT ON FUNCTION handle_new_auth_user IS 'Handles both agent and tenant user creation. First agent on a new site is automatically granted admin.';
