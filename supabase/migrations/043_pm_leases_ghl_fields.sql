...
ADD COLUMN IF NOT EXISTS late_fee_is_percentage BOOLEAN DEFAULT false,
ADD COLUMN IF NOT EXISTS late_fee_amount DECIMAL(10,2) DEFAULT 50.00,
ADD COLUMN IF NOT EXISTS late_fee_percentage DECIMAL(5,2) DEFAULT 5.00,
ADD COLUMN IF NOT EXISTS late_fee_frequency TEXT DEFAULT 'per occurrence',
...
COMMENT ON COLUMN pm_leases.late_fee_is_percentage IS 'If true, charge percentage of rent; if false, charge fixed amount';
COMMENT ON COLUMN pm_leases.late_fee_amount IS 'Late fee fixed dollar amount (used when is_percentage is false)';
COMMENT ON COLUMN pm_leases.late_fee_percentage IS 'Late fee as percentage of rent (e.g., 5.00 for 5%, used when is_percentage is true)';
COMMENT ON COLUMN pm_leases.late_fee_frequency IS 'How late fee accrues: "per occurrence" or "per day"';
