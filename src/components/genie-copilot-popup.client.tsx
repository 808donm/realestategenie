  ...
  const speakText = useCallback(async (text: string) => {
    ...
    if ("speechSynthesis" in window) {
      ...
      if (clean) {
        ...
        const preferred =
          voices.find(
            (v) => /samantha|karen|moira|tessa|fiona|victoria|zira/i.test(v.name) && v.lang.startsWith("en"),
      ...
      try {
        const res = await fetch("/api/genie/copilot", {
          ...
          body: JSON.stringify({
            ...
            selectedProperty: (() => {
              try {
                return JSON.parse(sessionStorage.getItem("hoku_selected_property") || "null");
              } catch {
                return null;
              }
            })(),
            selectedLead: (() => {
              try {
                return JSON.parse(sessionStorage.getItem("hoku_selected_lead") || "null");
              } catch {
                return null;
              }
            })(),
        ...
        {messages.length === 0 && !loading && (
          <div style={{ textAlign: "center", padding: 30, color: "hsl(var(--muted-foreground))" }}>
            ...
            <div style={{ fontSize: 12, marginTop: 4 }}>
              Aloha! I can search properties, create tasks, run calculators, draft emails, and more. What can I help
              with?
            </div>
          </div>
        )}
                ...
                <div style={{ marginTop: 8 }}>
                        ...
                        <div style={{ marginTop: 6 }}>
                          ...
                          >
                            <strong style={{ color: "#1e40af" }}>
                              {aiScored
                                ? `Found ${total} properties — showing top ${Math.min(props.length, 15)} most likely sellers`
                          ...
                          >
                            {props.slice(0, 15).map((p: any, j: number) => {
                              ...
                              const absentee =
                                p.owner?.absenteeOwnerStatus === "A" ||
                                p.owner?.absenteeOwnerStatus?.includes?.("Absentee");
                                ...
                                >
                                  ...
                                  >
                                    <div style={{ flex: 1 }}>
                                      ...
                                      {reasoning && (
                                        <div
                                          style={{ fontSize: 10, color: tc.text, marginTop: 3, fontStyle: "italic" }}
                                        >
                                          {reasoning}
                                        </div>
                                      )}
                  ...
                  {msg.actionResult.data?.event && (
                    ...
                    >
                      ...
                      {msg.actionResult.data.event.start_at &&
                        ` | ${new Date(msg.actionResult.data.event.start_at).toLocaleString()}`}
                    </div>
                  )}
                  ...
                  {msg.actionResult.data?.draft && (
                    ...
                    >
                      {msg.actionResult.data.draft.subject && (
