...
export async function getTrestleClient(supabase: SupabaseClient, agentId: string): Promise<TrestleClient | null> {
...
export async function updateTrestleSyncTime(supabase: SupabaseClient, agentId: string): Promise<void> {
  await supabase
    .from("integrations")
