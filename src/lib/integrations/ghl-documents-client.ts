...
export async function sendLeaseViaGHLDirect(
  ...
  // Template merge fields use {{contact.lease-property-address}} format with hyphens
  const customFieldsToUpdate: Record<string, string> = {
    'lease-property-address': leaseData.property_address,
    'lease-property-city': leaseData.property_city,
    'lease-property-state': leaseData.property_state,
    'lease-property-zipcode': leaseData.property_zipcode,
    'lease-start-date': leaseData.start_date,
    'lease-end-date': leaseData.end_date,
    'lease-monthly-rent': leaseData.monthly_rent.toString(),
    'lease-security-deposit': leaseData.security_deposit.toString(),
    'lease-rent-due-day': (leaseData.rent_due_day || 1).toString(),
    'lease-notice-days': (leaseData.notice_period_days || 30).toString(),
    'lease-late-grace-days': (leaseData.late_grace_days || 5).toString(),
    'lease-late-fee-type-fixed': leaseData.late_fee_is_percentage ? 'No' : 'Yes',
    'lease-late-fee-type-percentage': leaseData.late_fee_is_percentage ? 'Yes' : 'No',
    'lease-late-fee-amount': (leaseData.late_fee_amount || 50).toString(),
    'lease-late-fee-percentage': (leaseData.late_fee_percentage || 5).toString(),
    'lease-late-fee-frequency-occurence': (leaseData.late_fee_frequency || 'per occurrence') === 'per occurrence' ? 'Yes' : 'No',
    'lease-late-fee-frequency-daily': (leaseData.late_fee_frequency || 'per occurrence') === 'per day' ? 'Yes' : 'No',
    'lease-nsf-fee': (leaseData.nsf_fee || 35).toString(),
    'lease-deposit-return-days': (leaseData.deposit_return_days || 60).toString(),
    'lease-occupants': leaseData.occupants || '',
    'lease-subletting-yes': leaseData.subletting_allowed ? 'Yes' : 'No',
    'lease-subletting-no': leaseData.subletting_allowed ? 'No' : 'Yes',
    'lease-pets-yes': leaseData.pets_allowed ? 'Yes' : 'No',
    'lease-pets-no': leaseData.pets_allowed ? 'No' : 'Yes',
    'lease-pet-count': (leaseData.pet_count || 0).toString(),
    'lease-pet-types': leaseData.pet_types || '',
    'lease-pet-weight-limit': leaseData.pet_weight_limit || '',
    'lease-pet-deposit': (leaseData.pet_deposit || 0).toString(),
    'lease-landlord-notice-address': leaseData.landlord_notice_address || '',
  };
