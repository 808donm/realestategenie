 ...
 */
export async function getCrimeIndicesByState(
  stateAbbrev: string,
  year?: number,
): Promise<CrimeIndices | null> {
  ...
  try {
    ...
    const match = results.find((r: any) => r.year === targetYear)
    ...
    return {
      crimeIndex: rateToIndex(
        rate((match.violent_crime || 0) + (match.property_crime || 0)),
        "violent-crime",
      ),
 ...
 */
export async function getCrimeIndicesByFips(
  fips: string,
): Promise<CrimeIndices | null> {
