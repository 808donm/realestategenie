...
export function mapRealieToAttomShape(parcel: RealieParcel): any {
  ...
  const fips = parcel.fipsState && parcel.fipsCounty ? `${parcel.fipsState}${parcel.fipsCounty}` : undefined;
  ...
  if (ownerOccupied === undefined) {
    ...
    if (ownerAddr && propAddr) {
      ...
      ownerOccupied =
        parcel.ownerCity.toUpperCase() === parcel.city.toUpperCase() &&
  ...
  const pricePerSqft =
    parcel.transferPrice && parcel.buildingArea ? Math.round(parcel.transferPrice / parcel.buildingArea) : undefined;
  ...
  const transferDate = formatYMD(parcel.transferDate) || parcel.transferDateObject || parcel.transferDate;
  ...
  return {
    ...
    address: {
      oneLine:
        parcel.addressFullUSPS ||
        parcel.addressFull ||
        `${parcel.address}, ${parcel.city}, ${parcel.state} ${parcel.zipCode}`,
    ...
    owner: {
      ...
      corporateIndicator: (parcel.ownerComCount && parcel.ownerComCount > 0) || parcel.buyerIDCode === "CO" ? "Y" : "N",
      absenteeOwnerStatus: ownerOccupied === false ? "A" : ownerOccupied === true ? "O" : undefined,
      mailingAddressOneLine:
        parcel.ownerAddressFull ||
        (parcel.ownerAddressLine1
          ? [parcel.ownerAddressLine1, parcel.ownerCity, parcel.ownerState, parcel.ownerZipCode]
              .filter(Boolean)
              .join(", ")
          : undefined),
    ...
    avm: parcel.modelValue
      ...
      : undefined,
    sale:
      parcel.transferPrice || parcel.transferDate || parcel.ownershipStartDate
        ? {
            amount: {
        ...
        : undefined,
    mortgage:
      parcel.totalLienBalance != null ||
      parcel.lenderName ||
      parcel.totalLienCount != null ||
      parcel.totalFinancingHistCount != null
        ? {
    ...
    homeEquity:
      parcel.equityCurrentEstBal != null || parcel.LTVCurrentEstCombined != null
        ? {
            ...
            outstandingBalance:
              parcel.modelValue != null && parcel.equityCurrentEstBal != null
    ...
    foreclosure:
      parcel.forecloseCode || parcel.forecloseRecordDate || parcel.auctionDate
        ? {
...
export class RealieClient {
  ...
  private async request<T>(
    ...
    if (!response.ok) {
      ...
      console.error(`[Realie] API request FAILED (${response.status}) for ${url}:`, errorText);
 ...
 */
export function mapAttomParamsToRealie(endpoint: string, params: Record<string, any>): RealieSearchParams | null {
